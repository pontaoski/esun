...
import Capability.Funds (class Funds, adjustBalance, transferMoney)
...
component =
        ...
        contents = HH.div [ css ["flex", "flex-col", "items-center"] ]
            [ HH.div [ css ["flex", "flex-col", "space-y-4"] ]
                [ conditional (state.requestToWho == (Username "")) $ HH.div_
                    [ HH.label [ HP.for "username" ] [ HH.text "Target:" ]
                ...
                , HH.input [ HP.type_ HP.InputSubmit, HP.value "Adjust Balance", HE.onClick $ \_ -> Submit ]
                ]
            , case state.error of
                Just (Left err) ->
    ...
    handleAction = case _ of
        ...
        Submit -> do
            target <- H.gets _.toWho
            iron <- H.gets _.iron
            diamonds <- H.gets _.diamond
            res <- adjustBalance { iron, diamonds, target }
