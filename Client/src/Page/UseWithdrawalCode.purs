...
import Capability.DepositCode (class DepositCode, useWithdrawalCode)
...
type State =
    { myUser :: MyProfile
    , code :: String
    , status :: Tripart { iron :: Int, diamonds :: Int, password :: String }
    }
    ...
    render state =
        ...
        contents = HH.div [ css ["flex", "flex-col", "items-center", "space-y-4"] ]
            ...
            , case state.status of
                ...
                Result (Right { iron, diamonds, password }) ->
                    HH.p_
                        [ HH.text $
                            "You withdrew " <> (show iron) <> "i " <> (show diamonds) <> "d from your account."
                        , HH.br_
                        , HH.text $
                            "Use the password " <> password <> " to claim your funds ingame"
                        ]
                Pending ->
                    HH.input [ HP.type_ HP.InputSubmit, HP.value "Redeem Withdrawal Code", HP.disabled true ]
                Nichts ->
                    HH.input [ HP.type_ HP.InputSubmit, HP.value "Redeem Withdrawal Code", HE.onClick $ \_ -> Submit ]
            ]
